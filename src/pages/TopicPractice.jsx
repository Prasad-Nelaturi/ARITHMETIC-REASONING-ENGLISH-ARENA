...
import { generatePracticeQuestion } from '../utils/aiGenerator.js'
import {
...
const TopicPractice = () => {
    ...
    const startTimeRef = useRef(0)         // ms timestamp when current run began
    const pausedRemainingRef = useRef(duration) // seconds left at last pause
    ...
    }, [topic, level, language.aiName, duration])
    ...
    useEffect(() => {
        ...
        // Only initialize start time if this is a fresh run (not resuming)
        // Resuming sets startTimeRef based on pausedRemainingRef
    ...
    /* =========================================================
       ...
       ========================================================= */
    const togglePause = () => {
        if (chosen !== null) return   // can't pause after answering
        if (!paused) {
            pausedRemainingRef.current = remaining
            setPaused(true)
        } else {
            // Resuming — hide the explanation panel again
            setShowExplanation(false)
            setPaused(false)
        }
    }
        ...
        <div className="min-h-screen w-full pb-6 pt-[68px]">
            <div className="w-full max-w-lg mx-auto px-3 flex flex-col gap-3">
                ...
                <div className="flex items-center justify-between gap-2">
                    <button
                        type="button"
                        onClick={() => navigate(`/arithmetic/${topic.id}?lang=${lang}`)}
                    ...
                    <div className="flex-1 min-w-0 text-center">
                        <div className="text-[9px] font-black tracking-[1.6px] uppercase text-emerald-300 leading-none">
                            Practice
                        </div>
                ...
                {/* ===== CONTROLS: language + level + time (single compact card) ===== */}
                <div className="rounded-xl bg-slate-900/80 border border-slate-500/20 p-2 flex flex-col gap-2">
                    ...
                    <div className="flex items-center justify-between gap-2">
                        <div className="flex items-center gap-1.5">
                            <Sparkles size={11} className="text-slate-500" strokeWidth={2.8} />
                            <span className="text-[9px] font-black tracking-wider uppercase text-slate-500">
                                Level
                            </span>
                        </div>
                ...
                {loading && (
                    <div className="flex flex-col items-center gap-2 py-12 rounded-xl bg-slate-900/60 border border-slate-500/20">
                        <Loader2 size={24} className="animate-spin text-emerald-300" strokeWidth={2.6} />
                    ...
                    <div className="rounded-xl bg-slate-900/80 border border-slate-500/25 p-3 sm:p-4">
                        ...
                        {(answered || paused) && (
                            <div className="flex flex-col gap-2.5">
                                {/* Feedback row — only when actually answered */}
                                {answered && (
                                    <>
                                        {timedOut ? (
                                            <div className="flex items-center gap-2 px-2.5 py-2 rounded-lg bg-amber-500/15 border border-amber-500/40 text-amber-300 text-[11px] font-black">
                                                ...
                                                <span>Time's up — correct answer is <span className="text-amber-200">{question.correct}</span></span>
                                            </div>
                                        ) : isCorrect ? (
                                ...
                                {/* Paused notice — only when paused and not answered */}
                                {!answered && paused && (
                                ...
                                {/* Next Question — only after actually answering */}
                                {answered && (
                                    <button
                                        type="button"
                                        onClick={loadQuestion}
                                        className="w-full flex items-center justify-center gap-1.5 py-2.5 text-[11px] font-black tracking-wider uppercase text-white bg-gradient-to-br from-emerald-500 to-teal-600 border border-emerald-400/60 rounded-lg shadow-[0_6px_18px_rgba(16,185,129,0.35)] active:scale-[0.98]"
                                    >
