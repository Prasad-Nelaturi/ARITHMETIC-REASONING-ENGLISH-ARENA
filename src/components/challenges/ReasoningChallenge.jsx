...
import { Brain, CheckCircle2, XCircle, Lightbulb, Loader2, AlertCircle, RefreshCw } from 'lucide-react'
import { fetchQuestionSet } from '../../utils/questionService.js'

const QUESTIONS_PER_ROUND = 10

const ReasoningChallenge = ({
    ...
    useEffect(() => {
        ...
        setQuestions(null)
        setLoadError(null)
        fetchQuestionSet('reasoning', level, QUESTIONS_PER_ROUND)
    ...
    if (!questions) {
        return (
            <div className="flex flex-col items-center gap-3 py-10 px-5">
                <Loader2 size={28} className="animate-spin text-pink-300" strokeWidth={2.6} />
    ...
    const isTextAnswer = q.options.some((o) => String(o).length > 4)
            ...
            >
                <div className="flex items-center justify-between gap-1">
                    <div className="flex items-center gap-1.5 min-w-0">
                        <span className={`w-1.5 h-1.5 rounded-full shrink-0 ${isP1 ? 'bg-blue-500 shadow-[0_0_8px_#3b82f6]' : 'bg-pink-500 shadow-[0_0_8px_#ec4899]'}`} />
                        <span className={`text-[11px] font-black tracking-wide truncate max-w-[70px] uppercase ${isP1 ? 'text-blue-500' : 'text-pink-500'}`}>
                            {isP1 ? p1Name : p2Name}
                        </span>
                    </div>
                    <div className="flex gap-1 shrink-0">
                        <span className="text-[9px] font-extrabold px-1.5 py-0.5 rounded-md border border-green-400/20 bg-green-400/10 text-green-400 tabular-nums">✓ {correctCount[player]}</span>
                        <span className="text-[9px] font-extrabold px-1.5 py-0.5 rounded-md border border-red-400/20 bg-red-400/10 text-red-400 tabular-nums">✗ {wrongCount[player]}</span>
                    </div>
                </div>
                <div className={`grid gap-1.5 ${isTextAnswer ? 'grid-cols-1' : 'grid-cols-1 sm:grid-cols-2'}`}>
                    {q.options.map((opt, i) => {
                        ...
                        return (
                            <button
                                ...
                                disabled={locked}
                                className={`${isTextAnswer ? 'px-1.5 py-2.5 text-[clamp(10px,2.9vw,13px)]' : 'px-1 py-3 text-[clamp(14px,3.8vw,18px)]'} font-black text-white rounded-[10px] cursor-pointer transition-all duration-150 touch-manipulation break-words leading-tight ${showCorrect
    ...
    return (
        <div className="flex flex-col items-center gap-3 w-full">
                    ...
                    {questions.map((_, i) => (
                        <span
                            ...
                            className={`w-[7px] h-[7px] rounded-full transition-all duration-250 ${i < idx
                                    ...
                                    : i === idx
                                        ? 'bg-gradient-to-br from-pink-500 to-violet-500 shadow-[0_0_10px_rgba(236,72,153,0.8)] scale-150'
                                        : 'bg-slate-500/20'
                                }`}
                        />
                    ))}
            ...
            <div className="inline-flex items-center gap-1.5 text-[10px] font-black tracking-[1.4px] uppercase text-pink-300 bg-pink-500/15 border border-pink-500/35 px-2.5 py-1 rounded-full">
                <Brain size={11} strokeWidth={3} />
                <span>{q.topic}</span>
            </div>

            <div className="text-[clamp(12px,3.4vw,15px)] font-extrabold text-slate-100 text-center leading-normal px-2 min-h-[54px] flex items-center justify-center tracking-tight">
                {q.question}
            </div>
            ...
            {feedback ? (
                <div className={`flex items-center gap-1.5 px-3.5 py-2 rounded-[10px] text-xs font-extrabold text-center tracking-tight max-w-full animate-pop ${feedback.correct ? 'bg-green-500/15 border border-green-500/40 text-green-400' : 'bg-red-500/15 border border-red-500/40 text-red-300'
                    }`}>
                    {feedback.correct ? (
                        <><CheckCircle2 size={14} strokeWidth={3} /><span>Correct! +1 for {p1Name}</span></>
                    ) : (
                        <><XCircle size={14} strokeWidth={3} /><span>Wrong! Answer was "{feedback.correctValue}"</span></>
                    )}
                </div>
            ) : (
